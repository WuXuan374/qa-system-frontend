...
const QuestionAnswering = () => {
  ...
  // actions
  const getHints = (lang) => {
    triggerAPIRequest(`${apis.hints}/?lang=${lang}`, HTTP_GET)
      .then((data) => {
        const { hints } = data
        setHints(hints)
      })
  ...
  return (
    ...
    >
      <BodyContainer>
        <Card style={{ margin: 10 }} className="body-header">
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <Radio.Group onChange={onDatasetChange} defaultValue="en">
              <Radio.Button value="en">英文数据集</Radio.Button>
              <Radio.Button value="zh">中文数据集</Radio.Button>
            </Radio.Group>
            <AutoComplete
              value={question}
